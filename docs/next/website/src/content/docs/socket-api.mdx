...
`PaneInfo` also exposes an optional `foreground_in_linked_worktree` boolean: `true` when `foreground_cwd` resolves inside a linked git worktree (where the agent is actually operating, which can differ from the space's checkout identity), `false` when it does not, and omitted until Herdr has classified the directory. It is a periodically refreshed convenience derived from `foreground_cwd`, not authoritative for a working directory the agent changes between refreshes.
