...
fn sidebar_scrollbars_use_proportional_shared_geometry_and_drag() {
    ...
    // Enough entries and height that both the spaces and agents sections
    // overflow even after the agents pane reserves rows for the status legend at
    // the bottom, so the proportional thumbs stay taller than one cell.
    for index in 2..=40 {
    ...
    state.set_pane_surface(surface());
    state.compose(106, 40).expect("overflowing sidebars");
