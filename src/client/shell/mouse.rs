...
impl ClientShellState {
    ...
    /// Identifier of the sidebar entry under `point`, if any — a workspace card
    /// or an agent-panel row. Used only to decide when a hover tooltip should
    /// appear or disappear, so it lists exactly the entries that can produce one
    /// (see the hover-tooltip collection in `render_workspace_rows` /
    /// `render_agent_row`).
    fn sidebar_hover_key(&self, point: (u16, u16)) -> Option<String> {
        ...
        {
            return Some(format!("w:{}", hit.workspace_id));
        }
    ...
    pub(super) fn handle_mouse(&mut self, mouse: MouseEvent, outcome: &mut ClientShellInput) {
        ...
        self.last_mouse_pos = Some(point);
        // Repaint when a mouse *move* changes the hovered sidebar entry, so its
        // tooltip appears or disappears; plain motion over the sidebar is
        // otherwise render-neutral and would not trigger a compose. Non-motion
        // events (clicks, scrolls) drive their own repaint, so they must not be
        // forced to repaint merely because the pointer rests on an entry.
        if self.overlay.is_none() && matches!(mouse.kind, MouseEventKind::Moved) {
            let key = self.sidebar_hover_key(point);
            if key != self.hovered_sidebar_key {
                self.hovered_sidebar_key = key;
                outcome.repaint = true;
            }
        }
