...
/// Flatten a sidebar entry's resolved rows into one untruncated line for a
/// collapsed-rail hover tooltip. The collapsed rail shows only a number and a
/// status icon, so the tooltip carries everything the expanded row would — name,
/// branch, and trailing context — joined with the same separators, minus the
/// state-icon token (which the rail already draws).
fn collapsed_hover_line(
...
pub(crate) fn render_collapsed_sidebar(
        ...
        .enumerate()
    {
        ...
        if hover.is_some_and(|point| super::contains(rect, point)) {
            let bg = if selected {
            ...
            let spans = collapsed_hover_line(&rows, palette);
            if !spans.is_empty() {
                hits.hover_tooltips.push(HoverTooltip {
                    spans,
                    bg,
                    row: rect.y,
                    col: area.right(),
        ...
        .enumerate()
    {
        ...
        if hover.is_some_and(|point| super::contains(rect, point)) {
            let bg = if agent.focused {
                palette.active_row_bg
            } else {
                palette.sidebar_bg
            };
            let spans = super::agent_sidebar::agent_rows(snapshot, config, None)
                .into_iter()
                .find(|row| row.pane_id == pane_id)
                .map(|row| collapsed_hover_line(&row.rows, palette))
                .unwrap_or_default();
            if !spans.is_empty() {
                hits.hover_tooltips.push(HoverTooltip {
                    spans,
...
pub(in crate::client::shell) fn render_workspace_rows(
    ...
    palette: &Palette,
    hover: Option<(u16, u16)>,
    tooltips: &mut Vec<HoverTooltip>,
) {
    // Whether the mouse is over any row of this entry: hovering any row reveals
    // the tooltips for every truncated row of the entry, so a space's name and
    // its branch both expand together.
    let over_entry = hover.is_some_and(|point| super::contains(area, point));
    // The entry's background, reused for the row fill below and for every
    // tooltip box so hovering preserves it exactly.
    ...
    for (row_index, row) in rows.iter().enumerate() {
        ...
        let max_width = area.right().saturating_sub(2).saturating_sub(x) as usize;
        let spans = crate::ui::resolved_token_spans(
            row,
            icon,
            state_text_style,
        ...
        if over_entry {
            // Recompute this row without a width limit; if the untruncated text
            // differs, the row was truncated and earns a tooltip box.
            ...
            if spans_text(&full) != spans_text(&spans) {
                tooltips.push(HoverTooltip {
