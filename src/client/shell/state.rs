...
/// A floating label painted on top of the composed frame when the mouse hovers a
/// sidebar entry whose text was truncated to fit the column. Each tooltip is one
/// right-sized single-line box wrapping just its own text (plus one column of
/// padding on each side); an entry with several truncatable rows (e.g. a space's
/// name and its branch) contributes one independently sized box per row instead
/// of one box stretched to the widest.
pub(super) struct HoverTooltip {
    /// Full, untruncated spans making up this box's one line, each paired with
    /// the exact style it has in the sidebar so the box matches the truncated
    /// text's color and weight.
    pub(super) spans: Vec<(String, ratatui::style::Style)>,
    /// Background of the hovered row, reused so hovering never changes it (a
    /// focused space keeps its brighter fill; an idle one keeps the sidebar bg).
    pub(super) bg: ratatui::style::Color,
    /// Screen row this box aligns to.
    pub(super) row: u16,
...
pub(super) struct ShellHitMap {
    ...
    pub(super) hover_tooltips: Vec<HoverTooltip>,
...
pub(crate) struct ClientShellState {
    ...
    /// event. Drives hover tooltips for truncated sidebar labels.
    pub(super) last_mouse_pos: Option<(u16, u16)>,
    /// Identifier of the sidebar entry currently under the mouse, if any. Used
    /// to force a repaint only when the hovered entry changes (so a tooltip
    /// appears/disappears), not on every render-neutral mouse move.
    pub(super) hovered_sidebar_key: Option<String>,
