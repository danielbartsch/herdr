    ...
    pub(crate) fn compose(&mut self, cols: u16, rows: u16) -> Option<FrameData> {
        ...
        self.hits = render::render_shell(
            ...
            render::ShellRenderState {
                ...
                // Only collect sidebar hover tooltips when no overlay covers the
                // sidebar; a menu or modal owns the surface otherwise.
                hover: self
                    .overlay
                    .is_none()
                    .then_some(self.last_mouse_pos)
        ...
        // Paint sidebar hover tooltips last, over the composed panes. Only ever
        // populated when no overlay is active (see the `hover` gate above), so
        // this never fights a menu or modal for the surface.
        if !self.hits.hover_tooltips.is_empty() {
            if let Some(mut composed) = frame.to_ratatui_buffer() {
                for tooltip in &self.hits.hover_tooltips {
                    render::render_hover_tooltip(&mut composed, tooltip);
                }
