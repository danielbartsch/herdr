...
pub(super) struct ShellRenderState<'a> {
    ...
    /// Mouse position (column, row) when a sidebar hover tooltip should be
    /// collected for the entry under it, or `None` to collect nothing.
...
/// Paint one hover tooltip box as a floating single-line label over the already
/// composed frame. Called once per box so each truncated sidebar row gets its
/// own right-sized box instead of a single box wide enough for all of them. The
/// box is padded with one space on each side, aligned to the hovered row/column,
/// and clamped to stay on screen; it intentionally overflows the sidebar into
/// the panes, painting over whatever sits beneath it.
pub(super) fn render_hover_tooltip(buffer: &mut Buffer, tooltip: &HoverTooltip) {
    ...
    let text_w: u16 = tooltip
        ...
        .sum();
    let box_w = (text_w.saturating_add(2)).min(area.width);
    ...
    // Fill the box with the hovered row's own background first (blanking any
    // underlying pane glyphs), then paint the padded spans over it.
    let base = Style::default().bg(tooltip.bg);
