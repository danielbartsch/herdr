...
/// Concatenated text of a span list, used to detect whether truncation changed
/// the rendered text of a sidebar row.
fn spans_text(spans: &[ratatui::text::Span<'_>]) -> String {
    spans.iter().map(|span| span.content.as_ref()).collect()
}

/// Convert rendered spans into the `(text, style)` pairs a `HoverTooltip` holds,
/// so the floating box matches the truncated text's color and weight.
fn hover_tooltip_spans(spans: &[ratatui::text::Span<'_>]) -> Vec<(String, Style)> {
    spans
        .iter()
