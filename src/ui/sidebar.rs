...
/// Concatenated text of a laid-out row. Comparing a row drawn at the column's
/// width against the same row drawn unbudgeted tells us whether any token was
/// truncated or dropped, which is what makes a hover tooltip worth showing.
fn spans_text(spans: &[Span<'static>]) -> String {
    spans.iter().map(|span| span.content.as_ref()).collect()
}

/// Convert a laid-out row into tooltip spans, keeping each token's own style so
/// the expanded box matches the truncated text's color and weight.
fn hover_tooltip_spans(spans: &[Span<'static>]) -> Vec<(String, Style)> {
    spans
        .iter()
...
fn render_workspace_list(
    ...
    for card in cards {
        ...
        // Hovering anywhere on the card (any of its rows) expands the whole
        // card, so gather one candidate box per rendered row while drawing and
        // keep them only if some row actually had to cut its tokens.
        ...
        let mut card_tooltips: Vec<HoverTooltip> = Vec::new();
        let mut card_truncated = false;
        ...
        for (row_index, resolved) in rows.iter().enumerate() {
            ...
            let token_spans = resolved_token_spans(
                resolved,
                state_icon,
                state_text_style,
                name_style,
            ...
            if over_card {
                // Same row, laid out with no width budget: whatever it says
                // beyond the drawn row is what the column hid.
                let full_spans = resolved_token_spans(
                    resolved,
                    state_icon,
                    state_text_style,
                    name_style,
                ...
                card_truncated |= spans_text(&full_spans) != spans_text(&token_spans);
                card_tooltips.push(HoverTooltip {
                    spans: hover_tooltip_spans(&full_spans),
                    bg: card_bg,
                    row: row_y + row_index as u16,
                    col: card.rect.x + prefix_width,
                });
            }
        ...
        if card_truncated {
            hover_tooltips.append(&mut card_tooltips);
        }
...
fn render_agent_detail(
    ...
    for (index, detail) in details.iter().enumerate().skip(scroll) {
        ...
        // expands every row of it, but only when the column cut something.
        let over_entry = app.last_mouse_pos.is_some_and(|(mx, my)| {
            mx >= body.x
                && mx < body.x + body.width
                && my >= row_y
                && my < row_y.saturating_add(height)
        });
        let mut entry_tooltips: Vec<HoverTooltip> = Vec::new();
        let mut entry_truncated = false;

        for (row_index, resolved) in rows.iter().take(height as usize).enumerate() {
            let mut spans = vec![Span::raw(if row_index == 0 { " " } else { "   " })];
            let prefix_width: u16 = if row_index == 0 { 1 } else { 3 };
            let token_spans = resolved_token_spans(
                resolved,
                state_icon,
                status_style,
        ...
        if entry_truncated {
            hover_tooltips.append(&mut entry_tooltips);
        }
        row_y = row_y
...
"##,
    ...
    /// Narrow sidebar holding one card whose name and branch cannot fit, so the
    /// token rows have to cut something and a hover has a reason to expand them.
    fn truncating_workspace_list_app() -> (AppState, Rect) {
        let mut app = AppState::test_new();
        let mut ws = Workspace::test_new("workspace-with-a-very-long-name");
        ws.cached_git_branch = Some("feature/an-equally-long-branch".into());
        app.workspaces = vec![ws];
        app.active = Some(0);
        app.selected = 0;
        app.mode = Mode::Terminal;
    ...
    fn draw_workspace_list_tooltips(app: &AppState, area: Rect) -> Vec<super::super::HoverTooltip> {
    ...
    #[test]
    fn hovering_a_truncated_card_expands_every_row_of_it() {
        let (mut app, area) = truncating_workspace_list_app();
        let card = app.view.workspace_card_areas[0].rect;
        // Hovering the branch row, not the name row: the whole card expands.
        app.last_mouse_pos = Some((card.x + 1, card.y + 1));

        let tooltips = draw_workspace_list_tooltips(&app, area);

        assert_eq!(tooltips.len(), 2, "one box per rendered row");
        let text = |tooltip: &super::super::HoverTooltip| {
            tooltip
                .spans
                .iter()
        ...
        assert!(
            text(&tooltips[0]).contains("workspace-with-a-very-long-name"),
            "name row should be untruncated: {:?}",
            text(&tooltips[0])
        );
        assert!(
            text(&tooltips[1]).contains("feature/an-equally-long-branch"),
            "branch row should be untruncated: {:?}",
            text(&tooltips[1])
        );
        assert_eq!(tooltips[0].row, card.y);
        assert_eq!(tooltips[1].row, card.y + 1);
        // Boxes align to where each row's tokens start, past the row's indent.
        assert_eq!(tooltips[0].col, card.x + 1);
        assert_eq!(tooltips[1].col, card.x + 3);
    }

    #[test]
    fn card_that_fits_the_column_has_no_hover_tooltip() {
        let (mut app, _) = truncating_workspace_list_app();
        let wide = Rect::new(0, 0, 60, 6);
        app.view.workspace_card_areas[0].rect = Rect::new(0, 1, 60, 2);
        app.last_mouse_pos = Some((1, 1));

        assert!(draw_workspace_list_tooltips(&app, wide).is_empty());
    }

    #[test]
    fn mouse_off_the_card_leaves_it_truncated() {
        let (mut app, area) = truncating_workspace_list_app();
        let card = app.view.workspace_card_areas[0].rect;
        app.last_mouse_pos = Some((card.x, card.y + card.height));

        assert!(draw_workspace_list_tooltips(&app, area).is_empty());
    }
