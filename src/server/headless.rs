...
impl HeadlessServer {
    ...
    fn handle_client_input_events(
        ...
        let hover_before = self.app.state.sidebar_hover_target();
        ...
        // A sidebar hover tooltip is painted from the mouse position alone, so a
        // move that changes which entry is hovered repaints even when the motion
        // would otherwise be render-neutral.
        let sidebar_hover_changed = self.app.state.sidebar_hover_target() != hover_before;
        if sidebar_hover_changed && render_neutral_mouse_motion {
            if let Some(client) = self.clients.get_mut(&client_id) {
                client.request_semantic_redraw_after_input();
            }
        }
...
"#,
    ...
    #[tokio::test]
    async fn mouse_motion_onto_a_sidebar_card_requests_a_render() {
        // Plain mouse motion is render-neutral, but the sidebar hover tooltip is
        // painted from the mouse position alone. Without a render the tooltip of
        // the last clicked entry stays on screen no matter where the mouse goes.
        let mut server = test_headless_server();
        let workspace = crate::workspace::Workspace::test_new("a-space-with-a-long-name");
        server.app.state.workspaces = vec![workspace];
        server.app.state.ensure_test_terminals();
        server.app.state.active = Some(0);
        server.app.state.selected = 0;
        server.app.state.mode = crate::app::Mode::Terminal;
        server.app.state.view.sidebar_rect = ratatui::layout::Rect::new(0, 0, 26, 24);
        server.app.state.view.terminal_area = ratatui::layout::Rect::new(26, 0, 54, 24);
        server.app.state.view.workspace_card_areas = crate::ui::compute_workspace_card_areas(
            &server.app.state,
            server.app.state.view.sidebar_rect,
        );
        server.clients.insert(
            1,
            ClientConnection::new(
        ...
        // SGR mouse motion with no buttons held (32 + 3), 1-based coordinates.
        let motion = |col: u16, row: u16| ServerEvent::ClientInput {
            client_id: 1,
            data: format!("\x1b[<35;{};{}M", col + 1, row + 1).into_bytes(),
        };

        let pane_col = server.app.state.view.terminal_area.x + 2;
        assert!(
            !server.handle_server_event(motion(pane_col, card.y)),
            "motion inside a pane should stay render-neutral"
        );
        assert!(
            server.handle_server_event(motion(card.x + 1, card.y)),
            "motion onto a space card has to render its hover tooltip"
        );
        assert!(
            server.handle_server_event(motion(pane_col, card.y)),
            "motion off the card has to render to clear the tooltip"
        );
    }
