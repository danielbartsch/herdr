...
async fn focused_terminal_title_syncs_without_requesting_a_sidebar_render() {
    ...
    // This test exercises the path where the sidebar does not display the
    // terminal title, so a title sync must not request a sidebar render. Pin a
    // title-free sidebar config so the assertion stays valid regardless of the
    // bundled default (which shows an agent's stripped title in the agents pane).
    server.app.state.sidebar_agents = crate::config::AgentsSidebarConfig {
