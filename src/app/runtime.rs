        ...
        let changed = match event {
            ...
            crate::raw_input::RawInputEvent::Mouse(mouse) => {
                ...
                if self.state.popup_pane.is_some() || self.state.mouse_capture {
                    let hover_before = self.state.sidebar_hover_target();
                    self.handle_mouse(mouse);
                    // Crossing into or out of a sidebar entry shows or hides its
                    // hover tooltip, so that motion is not render-neutral.
                    changes_view |= self.state.sidebar_hover_target() != hover_before;
                } else {
                    self.state
