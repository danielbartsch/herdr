...
/// The sidebar entry the mouse is resting on. A hover tooltip expands a whole
/// space card or agent entry at once, so this identifies everything a plain
/// mouse move can change about the painted frame.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum SidebarHoverTarget {
    Space(usize),
    Agent(usize, usize, crate::layout::PaneId),
}

impl AppState {
    /// Which sidebar entry, if any, the last known mouse position hovers.
    ///
    /// Plain mouse motion is treated as render-neutral (pane apps answer it
    /// through their own PTY output), so without this the hover tooltip would
    /// only appear or clear on the next unrelated repaint — leaving the last
    ...
    pub(crate) fn sidebar_hover_target(&self) -> Option<SidebarHoverTarget> {
        ...
        if let Some(card) = hovered_card {
            return Some(SidebarHoverTarget::Space(card.ws_idx));
        }
        self.agent_detail_target_at(row)
            .map(|(ws_idx, tab_idx, pane_id)| SidebarHoverTarget::Agent(ws_idx, tab_idx, pane_id))
    }
...
mod tests {
    ...
    fn app_with_one_space_card() -> crate::app::App {
        let mut app = app_for_mouse_test();
    ...
    #[test]
    fn sidebar_hover_target_tracks_the_space_card_under_the_mouse() {
        let mut app = app_with_one_space_card();
        ...
        // Inside the terminal area, so no sidebar entry is hovered.
    ...
    #[test]
    fn mouse_motion_across_a_sidebar_entry_repaints_the_view() {
        let mut app = app_with_one_space_card();
        let card = app.state.view.workspace_card_areas[0].rect;
        let pane_col = app.state.view.terminal_area.x + 2;

        // Moving within a pane stays render-neutral: pane apps answer motion
        // through their own output.
        assert!(!app.state.mode.mouse_motion_changes_view());
        app.state.last_mouse_pos = Some((pane_col, card.y));
        let hover_before = app.state.sidebar_hover_target();
        app.handle_mouse(mouse(MouseEventKind::Moved, pane_col + 1, card.y));
        assert_eq!(app.state.sidebar_hover_target(), hover_before);

        // Moving onto the card, and back off it, both have to repaint or the
        // tooltip would appear/linger until some unrelated redraw.
        let hover_before = app.state.sidebar_hover_target();
        app.handle_mouse(mouse(MouseEventKind::Moved, card.x + 1, card.y));
        assert_ne!(app.state.sidebar_hover_target(), hover_before);

        let hover_before = app.state.sidebar_hover_target();
        app.handle_mouse(mouse(MouseEventKind::Moved, pane_col, card.y));
        assert_ne!(app.state.sidebar_hover_target(), hover_before);
        assert_eq!(app.state.sidebar_hover_target(), None);
    }
